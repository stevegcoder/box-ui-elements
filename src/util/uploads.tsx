...
function getFile(file: UploadFile | UploadFileWithAPIOptions): UploadFile {
  if (doesFileContainAPIOptions(file)) {
    return ((file as any) as UploadFileWithAPIOptions).file;
  }

  return ((file as any) as UploadFile);
}
/**
...
function getDataTransferItem(item: DataTransferItem | UploadDataTransferItemWithAPIOptions): DataTransferItem {
  if (doesDataTransferItemContainAPIOptions(item)) {
    return ((item as any) as UploadDataTransferItemWithAPIOptions).item;
  }

  return ((item as any) as DataTransferItem);
}
/**
...
function getFileAPIOptions(file: UploadFile | UploadFileWithAPIOptions): UploadItemAPIOptions {
  if (doesFileContainAPIOptions(file)) {
    return ((file as any) as UploadFileWithAPIOptions).options || DEFAULT_API_OPTIONS;
  }
...
function getDataTransferItemAPIOptions(item: DataTransferItem | UploadDataTransferItemWithAPIOptions): UploadItemAPIOptions {
  if (doesDataTransferItemContainAPIOptions(item)) {
    return ((item as any) as UploadDataTransferItemWithAPIOptions).options || DEFAULT_API_OPTIONS;
  }
...
function getFileLastModifiedAsISONoMSIfPossible(file: UploadFile): string | null | undefined {
  if ( // $FlowFixMe https://github.com/facebook/flow/issues/6131
  file.lastModified && (typeof file.lastModified === 'string' || typeof file.lastModified === 'number' || file.lastModified instanceof Date)) {
...
function tryParseJson(maybeJson: string): Object | null | undefined {
  try {
...
function isDataTransferItemAFolder(itemData: UploadDataTransferItemWithAPIOptions | DataTransferItem): boolean {
  ...
  const entry = getEntryFromDataTransferItem(((item as any) as DataTransferItem));
...
async function getFileFromDataTransferItem(itemData: UploadDataTransferItemWithAPIOptions | DataTransferItem): Promise<UploadFile | UploadFileWithAPIOptions | null> {
  const item = getDataTransferItem(itemData);
  const entry = getEntryFromDataTransferItem(((item as any) as DataTransferItem));
  ...
  if (doesDataTransferItemContainAPIOptions(itemData)) {
    return {
      file: ((file as any) as UploadFile),
...
function getFileId(file: UploadFileWithAPIOptions | UploadFile, rootFolderId: string): string {
  if (!doesFileContainAPIOptions(file)) {
    return ((file as any) as UploadFile).name;
  }

  const fileWithOptions = ((file as any) as UploadFileWithAPIOptions);
...
function getDataTransferItemId(itemData: DataTransferItem | UploadDataTransferItemWithAPIOptions, rootFolderId: string): string {
  const item = getDataTransferItem(itemData);
  const {
    name
