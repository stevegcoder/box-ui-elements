...
import { CLIENT_NAME_CONTENT_UPLOADER, DEFAULT_HOSTNAME_API, DEFAULT_HOSTNAME_UPLOAD, DEFAULT_ROOT, ERROR_CODE_UPLOAD_FILE_LIMIT, STATUS_COMPLETE, STATUS_ERROR, STATUS_IN_PROGRESS, STATUS_PENDING, VIEW_ERROR, VIEW_UPLOAD_EMPTY, VIEW_UPLOAD_IN_PROGRESS, VIEW_UPLOAD_SUCCESS } from '../../constants';
import { StringMap, Token, UploadDataTransferItemWithAPIOptions, UploadFileWithAPIOptions, UploadItem, View, UploadItemAPIOptions, UploadFile} from '../../types';
import { getTypedFileId, getTypedFolderId } from '../../util/file';
import { getDataTransferItemAPIOptions, getDataTransferItemId, getFile, getFileAPIOptions, getFileFromDataTransferItem, getFileId, isDataTransferItemAFolder, isMultiputSupported } from '../../util/uploads';
import '../base.scss';
import '../fonts.scss';
...
class ContentUploader extends Component<Props, State> {
    ...
    addFolderDataTransferItemsToUploadQueue = async (
        ...
        // $FlowFixMe
        const fileAPIOptions: any = getDataTransferItemAPIOptions(newItems[0]);
    ...
    addFilesWithRelativePathToQueue(files: Array<UploadFileWithAPIOptions | File>, itemUpdateCallback: Function) {
        ...
        const { rootFolderId } = this.props;
        // $FlowFixMe
        const fileAPIOptions: any = getFileAPIOptions(files[0]);
        const { folderId = rootFolderId } = fileAPIOptions;
        const folderUpload = this.getFolderUploadAPI(folderId);
