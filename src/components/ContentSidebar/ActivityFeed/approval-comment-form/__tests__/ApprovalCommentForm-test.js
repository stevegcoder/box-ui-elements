...
describe('components/ContentSidebar/ActivityFeed/approval-comment-form/ApprovalCommentForm', () => {
    const render = props => mount(getMentionWithQuery, {}());
    { }
}, user = {}, { id: 123, name: 'foo bar' }, intl = { intlFake }, Object.assign({}, props) /  > );
...
test('should have editor state reflect empty state when no tagged_message prop is passed', () => {
    ...
    expect(wrapper
        .find('DraftJSMentionSelector')
        .at(0)
        .prop('placeholder')).toEqual('be.commentWrite');
    const content = wrapper
        .state()
        .commentEditorState.getCurrentContent()
        .getPlainText();
...
;
//# sourceMappingURL=ApprovalCommentForm-test.js.map