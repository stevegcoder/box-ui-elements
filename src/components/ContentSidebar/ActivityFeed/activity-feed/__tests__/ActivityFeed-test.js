...
const getWrapper = props => shallow(file, { file }, currentUser = { currentUser }, Object.assign({}, props) /  > );
describe('components/ContentSidebar/ActivityFeed/activity-feed/ActivityFeed', () => {
    ...
    test('should correctly render empty state', () => {
        ...
        const wrapper = shallow(file, { file }, currentUser = { currentUser }, comments = { items }, tasks = { items }, versions = { items } /  > );
        expect(wrapper).toMatchSnapshot();
    });
    ...
    test('should not render approval comment form if only comment submit handler is not passed in', () => {
        ...
        const wrapper = shallow(file, { file }, currentUser = { currentUser } /  > );
        expect(wrapper).toMatchSnapshot();
    });
    test('should not render approval comment form if comment permissions are not present', () => {
        file.permissions.can_comment = false;
        const wrapper = shallow(file, { file }, currentUser = { currentUser }, onCommentCreate = { jest, : .fn() } /  > );
        expect(wrapper).toMatchSnapshot();
    });
    ...
    test('should set scrollTop to be the scrollHeight if feedContainer exists and prevProps feedItems is undefined and this.props.feedItems is defined', () => {
        const wrapper = shallow(currentUser, { currentUser }, feedItems = { [{ type: 'comment' }]:  } /  > );
        const instance = wrapper.instance();
        instance.feedContainer = {
            scrollTop: 0,
            scrollHeight: 100,
    ...
    test('should set scrollTop to be the scrollHeight if more feedItems are added', () => {
        const wrapper = shallow(currentUser, { currentUser }, feedItems = { [{ type: 'comment' }, { type: 'comment' }]:  } /  > );
        const instance = wrapper.instance();
        instance.feedContainer = {
            scrollTop: 0,
            scrollHeight: 100,
    ...
    test('should hide input when approvalCommentFormCancelHandler is called', () => {
        const wrapper = shallow(currentUser, { currentUser }, onCommentCreate = { jest, : .fn() } /  > );
        const instance = wrapper.instance();
        instance.approvalCommentFormFocusHandler();
        expect(wrapper.state('isInputOpen')).toBe(true);
    ...
    test('should call create comment handler and close input on valid comment submit', () => {
        ...
        const wrapper = shallow(file, { file }, feedItems = { feedItems }, currentUser = { currentUser }, onCommentCreate = { createCommentSpy }
            /  > );
        const instance = wrapper.instance();
    ...
    test('should call create task handler and close input on valid task submit', () => {
        ...
        const wrapper = shallow(file, { file }, feedItems = { feedItems }, currentUser = { currentUser }, onCommentCreate = { jest, : .fn() }, onTaskCreate = { createTaskSpy }
            /  > );
        const instance = wrapper.instance();
        const approvalCommentForm = wrapper.find('ApprovalCommentForm').first();
        instance.approvalCommentFormFocusHandler();
...
//# sourceMappingURL=ActivityFeed-test.js.map