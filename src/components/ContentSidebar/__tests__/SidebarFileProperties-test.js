...
import { getClassificationModal, } from '../SidebarFileProperties';
...
describe('components/ContentSidebar/SidebarFileProperties', () => {
    ...
    const getMountWrapper = props => mount(Object.assign({}, props) /  > );
    const props = {
        file: {
    ...
    describe('render()', () => {
        ...
        test('should render an error', () => {
            ...
            const wrapper = shallow(Object.assign({}, errorProps) /  > ).dive();
        ...
        test('should not render classification information if hasClassification is false', () => {
            const wrapper = getMountWrapper(Object.assign({}, props, { hasClassification: false }));
            expect(wrapper).toMatchSnapshot();
        });
        ...
        test('should render classification link only when given callback and has_upload permission is true', () => {
            ...
            const wrapper = getMountWrapper(Object.assign({}, classificationProps, { file: Object.assign({}, classificationProps.file, { permissions: {
                        can_upload: true,
                    }, metadata: null }) }));
            expect(wrapper).toMatchSnapshot();
        });
        test('should not render a tooltip if no banner policy body is provided', () => {
            const wrapper = getMountWrapper(Object.assign({}, classificationProps, { bannerPolicy: undefined, file: Object.assign({}, classificationProps.file, { permissions: {
                        can_upload: true,
                    }, metadata: null }) }));
            expect(wrapper).toMatchSnapshot();
        });
    ...
    describe('getClassificationModal()', () => {
        test('should not have onClassificationClick prop on SidebarFileProperties when can_upload is falsy', () => {
            expect(getClassificationModal(classificationProps.file, classificationProps.onClassificationClick)).toBeUndefined();
        });
        test('should have onClassificationClick prop on SidebarFileProperties when can_upload is true', () => {
            expect(getClassificationModal(Object.assign({}, classificationProps.file, { permissions: {
                    can_upload: true,
                } }), classificationProps.onClassificationClick)).toEqual(classificationProps.onClassificationClick);
        });
    });
});
//# sourceMappingURL=SidebarFileProperties-test.js.map