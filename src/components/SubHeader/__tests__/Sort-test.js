...
describe('components/SubHeader/Sort', () => {
    test('should render a button and menu with 4 menu items', () => {
        const wrapper = shallow(isLoaded, { false:  }, sortBy = "name", sortDirection = { SORT_ASC }, onSortChange = { jest, : .fn() } /  > );
    ...
    test('should render a select with 4 options', () => {
        ...
        expect(options
            .at(0)
            .childAt(1)
            .prop('id')).toBe(messages.nameASC.id);
        expect(options
            .at(1)
            .childAt(0)
            .text()).toBe('');
        expect(options
            .at(1)
            .childAt(1)
            .prop('id')).toBe(messages.nameDESC.id);
        expect(options
            .at(2)
            .childAt(0)
            .text()).toBe('');
        expect(options
            .at(2)
            .childAt(1)
            .prop('id')).toBe(messages.dateASC.id);
        expect(options
            .at(3)
            .childAt(0)
            .text()).toBe('');
        expect(options
            .at(3)
            .childAt(1)
            .prop('id')).toBe(messages.dateDESC.id);
    });
    test('should pass correct parameters when clicked', () => {
        const sort = jest.fn();
        const wrapper = shallow(isLoaded, { false:  }, sortBy = "name", sortDirection = { SORT_ASC }, onSortChange = { sort } /  > );
        const options = wrapper.find(MenuItem);
    ...
    test('should render a select with correct option selected', () => {
        const wrapper = shallow(isLoaded, { false:  }, sortBy = "date", sortDirection = { SORT_DESC }, onSortChange = { jest, : .fn() } /  > );
        const options = wrapper.find(MenuItem);
        expect(options).toHaveLength(4);
        expect(options
            .at(3)
    ...
    test('should render a non-disabled button when isLoaded is true', () => {
        const wrapper = mount(isLoaded, sortBy = "name", sortDirection = { SORT_ASC }, onSortChange = { jest, : .fn() } /  > );
