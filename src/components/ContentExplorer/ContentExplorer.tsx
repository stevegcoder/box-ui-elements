...
import '../base.scss';
import '../modal.scss';
import {SortBy, SortDirection, Token, DefaultView, StringMap} from '../../types';

type Props = { rootFolderId: string,
...
class ContentExplorer extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    this.rootElement = ((document.getElementById(this.id) as any) as HTMLElement);
    this.appElement = ((this.rootElement.firstElementChild as any) as HTMLElement);
  ...
  finishNavigation() {
    ...
    }: State = this.state; // If loading for the very first time, only allow focus if autoFocus is true
  ...
  fetchFolderSuccessCallback(collection: Collection, triggerNavigationEvent: boolean): void {
    ...
    const {
      ...
      name,
      boxItem
    }: Collection = collection; // New folder state
    ...
    this.unselect(); // Close any open modals
  ...
  fetchFolder = (id?: string, triggerNavigationEvent?: boolean = true) => {
    ...
    this.api.getFolderAPI().getFolder(folderId, limit, offset, sortBy, sortDirection, (collection: Collection) => {
  ...
  searchSuccessCallback = (collection: Collection) => {
    const {
      currentCollection
    }: State = this.state; // Unselect any rows that were selected
  ...
  search = (query: string) => {
    ...
    if (!query) {
      ...
      this.debouncedSearch.cancel(); // Query was cleared out, load the prior folder
  ...
  recentsSuccessCallback(collection: Collection, triggerNavigationEvent: boolean) {
    // Unselect any rows that were selected
    this.unselect(); // Set the new state and focus the grid for tabbing
  ...
  showRecents(triggerNavigationEvent: boolean = true): void {
    const {
      rootFolderId
    }: Props = this.props; // Reset search state, the view and show busy indicator
    ...
    this.api.getRecentsAPI().recents(rootFolderId, (collection: Collection) => {
  ...
  renameCallback = (nameWithoutExt: string, extension: string): void => {
    ...
    this.api.getAPI(type).rename(selected, name, (updatedItem: BoxItem) => {
      this.setState({
        isRenameModalOpen: false
      });
      this.refreshCollection();
  ...
  createFolderCallback = (name?: string): void => {
    ...
    this.api.getFolderAPI().create(id, name, (item: BoxItem) => {
      this.refreshCollection();
      this.select(item);
  ...
  render() {
    ...
    return <Internationalize language={language} messages={messages}>
                <div id={this.id} className={styleClassName} ref={measureRef}>
                    <div className="be-app-element" onKeyDown={this.onKeyDown} tabIndex={0}>
                        <Header view={view} isSmall={isSmall} searchQuery={searchQuery} logoUrl={logoUrl} onSearch={this.search} />
                        <SubHeader view={view} rootId={rootFolderId} isSmall={isSmall} rootName={rootName} currentCollection={currentCollection} canUpload={allowUpload} canCreateNewFolder={allowCreate} onUpload={this.upload} onCreate={this.createFolder} onItemClick={this.fetchFolder} onSortChange={this.sort} />
                        <Content view={view} rootId={rootFolderId} isSmall={isSmall} isMedium={isMedium} isTouch={isTouch} rootElement={this.rootElement} focusedRow={focusedRow} canSetShareAccess={canSetShareAccess} canShare={canShare} canPreview={canPreview} canDelete={canDelete} canRename={canRename} canDownload={canDownload} currentCollection={currentCollection} tableRef={this.tableRef} onItemSelect={this.select} onItemClick={this.onItemClick} onItemDelete={this.delete} onItemDownload={this.download} onItemRename={this.rename} onItemShare={this.share} onItemPreview={this.preview} onSortChange={this.sort} />
                        <Footer>
                            <Pagination offset={offset} onChange={this.paginate} pageSize={currentPageSize} totalCount={totalCount} />
                        </Footer>
                    </div>
                    {allowUpload && !!this.appElement ? <UploadDialog isOpen={isUploadModalOpen} currentFolderId={id} token={token} sharedLink={sharedLink} sharedLinkPassword={sharedLinkPassword} apiHost={apiHost} uploadHost={uploadHost} onClose={this.uploadSuccessHandler} parentElement={this.rootElement} appElement={this.appElement} onUpload={onUpload} requestInterceptor={requestInterceptor} responseInterceptor={responseInterceptor} /> : null}
                    {allowCreate && !!this.appElement ? <CreateFolderDialog isOpen={isCreateFolderModalOpen} onCreate={this.createFolderCallback} onCancel={this.closeModals} isLoading={isLoading} errorCode={errorCode} parentElement={this.rootElement} appElement={this.appElement} /> : null}
                    {canDelete && selected && !!this.appElement ? <DeleteConfirmationDialog isOpen={isDeleteModalOpen} onDelete={this.deleteCallback} onCancel={this.closeModals} item={selected} isLoading={isLoading} parentElement={this.rootElement} appElement={this.appElement} /> : null}
                    {canRename && selected && !!this.appElement ? <RenameDialog isOpen={isRenameModalOpen} onRename={this.renameCallback} onCancel={this.closeModals} item={selected} isLoading={isLoading} errorCode={errorCode} parentElement={this.rootElement} appElement={this.appElement} /> : null}
                    {canShare && selected && !!this.appElement ? <ShareDialog isOpen={isShareModalOpen} canSetShareAccess={canSetShareAccess} onShareAccessChange={this.changeShareAccess} onCancel={this.refreshCollection} item={selected} isLoading={isLoading} parentElement={this.rootElement} appElement={this.appElement} /> : null}
                    {canPreview && selected && !!this.appElement ? <PreviewDialog isOpen={isPreviewModalOpen} isTouch={isTouch} onCancel={this.closeModals} item={selected} currentCollection={currentCollection} token={token} parentElement={this.rootElement} appElement={this.appElement} onPreview={onPreview} onDownload={onDownload} canDownload={canDownload} cache={this.api.getCache()} apiHost={apiHost} appHost={appHost} staticHost={staticHost} sharedLink={sharedLink} sharedLinkPassword={sharedLinkPassword} contentPreviewProps={contentPreviewProps} requestInterceptor={requestInterceptor} responseInterceptor={responseInterceptor} /> : null}
                </div>
            </Internationalize>;
