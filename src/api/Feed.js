...
class Feed extends Base {
    constructor(options) {
         ...
         * @return {void}
         */
        this.updateTaskAssignmentSuccessCallback = (taskId, updatedAssignment, successCallback) => {
         ...
         * @return {void}
         */
        this.updateTask = (file, taskId, message, successCallback = noop, errorCallback, dueAt) => {
            if (!file.id) {
                throw getBadItemError();
            }
            this.id = file.id;
        ...
        this.updateTaskSuccessCallback = (task, successCallback) => {
            ...
            this.updateFeedItem(Object.assign({}, updates, { isPending: false }), task.id);
            if (!this.isDestroyed()) {
                successCallback(task);
            }
        };
         ...
         * @return {void}
         */
        this.deleteComment = (file, commentId, permissions, successCallback, errorCallback) => {
         ...
         * @return {void}
         */
        this.createTask = (file, currentUser, message, assignees, dueAt, successCallback, errorCallback) => {
            if (!file.id) {
                throw getBadItemError();
            }
            this.id = file.id;
         ...
         * @return {void}
         */
        this.deleteTask = (file, taskId, successCallback = noop, errorCallback = noop) => {
            if (!file.id) {
                throw getBadItemError();
            }
            this.id = file.id;
         ...
         */
        this.deleteFeedItem = (id, successCallback = noop) => {
            const cachedItems = this.getCachedItems(this.id);
            if (cachedItems) {
         ...
         * @return {void}
         */
        this.feedErrorCallback = (hasError = false, e, code) => {
            if (hasError) {
                this.hasError = true;
            }
         ...
         * @return {void}
         */
        this.addPendingItem = (id, currentUser, itemBase) => {
            if (!currentUser) {
         ...
         * @return {void}
         */
        this.createCommentSuccessCallback = (commentData, id, successCallback) => {
            ...
            this.updateFeedItem(Object.assign({}, commentData, { isPending: false }), id);
            if (!this.isDestroyed()) {
         ...
         * @return {void}
         */
        this.updateFeedItem = (updates, id) => {
            if (!this.id) {
                throw getBadItemError();
            }
            const cachedItems = this.getCachedItems(this.id);
         ...
         * @return {void}
         */
        this.createComment = (file, currentUser, text, hasMention, successCallback, errorCallback) => {
     ...
     * @return {void}
     */
    createTaskSuccessCallback(file, id, task, assignees, successCallback, errorCallback) {
        if (!file) {
            throw getBadItemError();
        }
        this.errorCallback = errorCallback;
     ...
     */
    createTaskAssignment(file, task, assignee) {
        if (!file.id) {
            throw getBadItemError();
        }
        this.id = file.id;
     ...
     * @return {Task}
     */
    appendAssignmentsToTask(task, assignments) {
        if (!assignments) {
            return task;
        }
     ...
     */
    createFeedError(message, title = messages.errorOccured) {
        return {
            error: { message, title },
        };
    }
     ...
     */
    addCurrentVersion(versions, file) {
